...
class SyncCard(QGroupBox):
    ...
    # --- firewall -------------------------------------------------------------
    def firewall_checked(self, chk: firewall.Check) -> None:
        """The dashboard runs the launch-time firewall check and owns the
        allow/remove control; we only use the result to explain an empty scan."""
